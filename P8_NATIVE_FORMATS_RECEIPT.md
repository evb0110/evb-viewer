...
The remaining gaps are browser caller/state handling for compact label representation, exact browser/native pixel parity, packaged OS checks, #518 Windows/cancellation instrumentation, selected-format/rollback native dialog journeys, and a same-basename crash-recovery collision replay. The protected two-tab crash-recovery replay passed, and full renderer insertion refresh now passes. Native macOS picker interception remains unavailable in the hidden package, so that dialog journey is explicitly unqualified. No owned Electron, Nuxt, sass, or esbuild process was left running by this lane. Other workers' processes were preserved. No hosted CI, release, issue closure, or Project status change was performed.
