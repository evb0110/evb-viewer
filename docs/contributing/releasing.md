...
The cutter keeps the ordinary candidate selection described below. If that candidate omits a required commit, it stops and names the newest green candidate containing all required commits when one exists, including the canary dispatch command when that candidate is not yet packaged, and otherwise says that none exists yet.

## What the command does

The cutter picks the release candidate, checks it, then publishes a release commit built from it.

- **Candidate.** The newest commit on freshly fetched `origin/main` whose `ci.yml` push run succeeded with a green `gates_ok`. Only push runs count; a `workflow_dispatch` run carries no `gates_ok`. A tip whose run is still in progress is skipped in favour of the newest finished green commit behind it, so a cut never waits on CI. When artifact evidence is mandatory (the patch default), the choice narrows further to the newest green commit that already has a successful `release-artifacts.yml` canary, so the release ships the commit that was packaged and smoke tested even when `main` has moved on. Only when no green commit has a canary does the newest green commit remain the candidate, so the canary check can print its dispatch command. If no commit qualifies, the cutter names every run it rejected and stops. The preflight prints the candidate SHA and its qualifying CI run id and URL.
...
- **Canary.** For patch releases, a successful `release-artifacts.yml` canary whose `head_sha` exactly equals the selected candidate is mandatory. The repository has no separately verifiable packaging-input tree hash, so a canary for another commit is not reusable; that is why candidate selection prefers a commit that already has one. Use `--artifact-evidence=advisory` on the cut or the read-only preflight to release the newest green commit and report missing, running, or failed candidate evidence without blocking. The command prints the run that satisfied or failed this check.
...
- `ci.yml`'s `gates_ok` is the one required verdict; the cutter releases only a commit whose push run has it green.
- The artifact canary (`release-artifacts.yml`) packages the current `main` tip on every platform once a day when `main` changed in the last 24 hours, including the packaged core-PDF journeys, so a packaging or verifier mistake surfaces within a day rather than at release time. A patch cut requires a green canary for its exact candidate.
