...
- `release:verify:checks` forces `CI=1` and runs the release-specific checks: Electron install verification, and the electron-builder ASAR-unpack policy. Pass `--scan-cleanup-identity` to add the 200-second canonical scan-cleanup identity test, which CI selects for affected scan-cleanup changes.
