...
- `release:cut` runs from a clean `main` checkout and releases the newest commit on freshly fetched `origin/main` that has both a successful `ci.yml` push run with a green `gates_ok` and a completed successful `ci-extended.yml` run for the same SHA. `gates_ok` no longer aggregates the Electron end-to-end, browser integration, native save or packaged Linux proofs, so both tiers are required; a lane the changed-area classifier skipped inside a successful extended run counts as success. It never waits on CI and never requires `HEAD` to be the tip.
...
The Project 6 binary-distribution flow keeps runtime input bytes out of future trees. `scripts/runtimeBinaryManifest.ts` pins each platform archive and its SHA-256. `pnpm run fetch:runtime-binaries` downloads the host-target archives into `.cache/runtime-binaries`, rechecks warm entries, verifies length and SHA-256, and stages them under `resources/`; the install `prepare` step, native bundlers, and release setup action use that path before build or packaging. To refresh those trees from Homebrew or MSYS2, run a bundler with `EVB_RUNTIME_BINARIES_FROM_SOURCE=1`, which skips the fetch. `pnpm run pack:runtime-binary-archives` then packs the current resource trees into `.devkit/tmp/runtime-archives/` for the repository owner to publish as the release assets named by the manifest. The tessdata archive is covered as data, while `packages/contracts/ocrLanguages.ts` remains the canonical language and per-model digest registry.

This shrinks the current tree and adds a build-time download. It does not shrink the repository's history. The roughly 638 MiB historical pack still holds every binary committed before the change. History is not rewritten, as decided in [#326](https://github.com/evb0110/evb-viewer/issues/326). Commit hashes, tags, release identities, Sentry references and existing clones keep working. Revisit that only on an explicit request backed by measured storage pressure and a full migration assessment.

Removing a third-party runtime file from a future tree, or fetching it during a build, reduces the current checkout or build inputs only. It does not rewrite the historical Git pack, existing commit IDs, tags, release identities, or Sentry references. The settled no-rewrite decision and its revisit condition are recorded in [Project 6 issue #326](https://github.com/evb0110/evb-viewer/issues/326). Do not rewrite history, force-push, retag, or migrate active clones unless a new explicit decision authorizes it with a measured storage and migration plan.
