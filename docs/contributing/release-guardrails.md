...
The Project 6 binary-distribution flow keeps runtime input bytes out of future trees. `scripts/runtimeBinaryManifest.ts` pins each platform archive and its SHA-256. The manual `Runtime Binaries` workflow builds the five target sets, verifies archive metadata, and uploads one archive per family and target as workflow artifacts. The repository owner publishes the new v2 assets after review; reusable Darwin arm64 and Windows x64 PDF-tool archives and the shared tessdata archive stay on v1. `pnpm run fetch:runtime-binaries` downloads the host-target archives into `.cache/runtime-binaries`, rechecks warm entries, verifies length and SHA-256, and stages them under `resources/`. The install `prepare` step and release setup action use that path before build or packaging. The tessdata archive is covered as data, while `packages/contracts/ocrLanguages.ts` remains the canonical language and per-model digest registry.
