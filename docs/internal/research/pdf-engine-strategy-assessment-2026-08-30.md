...
- Hayro's current minimum Rust version is 1.92. The report's dated baseline recorded EVB's 1.89.0 toolchain and workspace `rust-version = "1.87"`; the current source declares `rust-version = "1.89"` in the [workspace package](../../../native/Cargo.toml#L17-L20). A current Hayro prototype therefore includes a toolchain or dependency-version decision. See [Hayro's current README](https://github.com/LaurenzV/hayro/blob/5a5f0e247c970df948505ee0bb36e2df2504bf86/README.md#minimum-supported-rust-version-msrv) and the current [`rust-toolchain.toml:1-3`](../../../rust-toolchain.toml#L1-L3).
...
| PDF.js `saveDocument` walks pages and returns complete bytes. The dated EVB save transaction retry and source-byte fallback path is no longer tracked at the cited location. The current transaction instead computes a native route and returns a typed native-required failure in the [current save-route code](../../../app/modules/pdf-viewer/runtime/save/usePdfViewerSaveTransaction.ts#L376-L420). | Intrinsic API limit plus EVB coupling | The whole-byte result is a real PDF.js contract. The historical retry and fallback behavior is dated evidence, not a current-source claim. | A native append or staged writer can stream output and validate before publication. It must still preserve annotations and page identity. |
