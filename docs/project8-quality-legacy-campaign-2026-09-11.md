...
Reviewed source: `8652fce8` (`origin/project8/integration`)
...
The current integration CI run is `34632522496` for `8652fce8`. It was
cancelled before terminal quality acceptance, so it provides no current-tip
pass. The separate configured publish-chain drill is terminal-successful in
run `34630569082`; no terminal current-tip release artifact digest is claimed
here.
