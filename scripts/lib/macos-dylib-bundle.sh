...
macos_prune_unused_dylibs() {
  ...
  while [ "$index" -lt "${#files[@]}" ]; do
    ...
    local dependencies
    dependencies="$(macos_list_dylib_dependencies "$file")" || return 1
    local dependency
    for dependency in $dependencies; do
      macos_is_system_dylib_reference "$dependency" && continue
      local library="$destination_lib/$(basename "$dependency")"
      if [ ! -f "$library" ]; then
        echo "Error: Bundled dependency closure is missing $library (referenced by $file)" >&2
        return 1
      fi
      local retained=0
      local candidate
      for candidate in "${files[@]}"; do
        if [ "$candidate" = "$library" ]; then
          retained=1
          break
        fi
      done
