...
export const PRIVACY_MESSAGES = {
    'en': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer is the controller and uses Functional Software, Inc. d/b/a Sentry as its processor for optional client error reports. Desktop and browser reports are sent only with your consent. A report may contain a random Error ID, time, severity, diagnostic code, app release, operating system and runtime names and versions, EVB Viewer stack frames (file, function, line and column), and the error type and message after local paths, file names, URLs and quoted text are removed. For a native crash it records which process ended and why; the memory dump is never sent. It never contains document contents or text, local paths or logs, URLs or request data, console output, screenshots, attachments, account data, IP address, or identity. Reports are used only to diagnose crashes and product errors, never for analytics, advertising, profiling, AI, or training. Sentry stores events in the EU region for up to 90 days and may use its published subprocessors and the DPA and Standard Contractual Clauses for limited onward transfers. You can withdraw consent at any time to stop future reports. The EVB Viewer web service sends no server-side error reports.',
        },
        analytics: {
    ...
    'de': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer ist der Verantwortliche und nutzt Functional Software, Inc. d/b/a Sentry als Auftragsverarbeiter für optionale Fehlerberichte des Clients. Desktop- und Browserberichte werden nur mit deiner Einwilligung gesendet. Ein Bericht kann eine zufällige Fehler-ID, Zeitpunkt, Schweregrad, Diagnosecode, App-Version, Name und Version von Betriebssystem und Laufzeit, EVB-Viewer-Stackframes (Datei, Funktion, Zeile und Spalte) sowie Fehlertyp und Fehlermeldung enthalten, nachdem lokale Pfade, Dateinamen, URLs und zitierter Text entfernt wurden. Bei einem nativen Absturz wird festgehalten, welcher Prozess aus welchem Grund beendet wurde; das Speicherabbild wird nie gesendet. Er enthält niemals Dokumentinhalte oder Text, lokale Pfade oder Protokolle, URLs oder Anfragedaten, Konsolenausgaben, Bildschirmfotos, Anhänge, Kontodaten, IP-Adresse oder Identität. Die Berichte dienen nur der Diagnose von Abstürzen und Produktfehlern, nie Analyse, Werbung, Profiling, KI oder Training. Sentry speichert Ereignisse bis zu 90 Tage in der EU-Region und kann veröffentlichte Unterauftragsverarbeiter sowie den DPA und Standardvertragsklauseln für begrenzte Weiterübermittlungen nutzen. Du kannst die Einwilligung jederzeit widerrufen und damit künftige Berichte stoppen. Der EVB-Viewer-Webdienst sendet keine serverseitigen Fehlerberichte.',
        },
        analytics: {
    ...
    'es': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer es el responsable del tratamiento y utiliza Functional Software, Inc. d/b/a Sentry como encargado para los informes opcionales de errores del cliente. Los informes del escritorio y del navegador se envían solo con tu consentimiento. Un informe puede incluir un Error ID aleatorio, fecha y hora, gravedad, código de diagnóstico, versión de la aplicación, nombre y versión del sistema operativo y del entorno, marcos de pila de EVB Viewer (archivo, función, línea y columna) y el tipo y el mensaje del error después de eliminar rutas locales, nombres de archivo, URL y texto entre comillas. En un fallo nativo registra qué proceso terminó y por qué; el volcado de memoria nunca se envía. Nunca incluye documentos o texto, rutas o registros locales, URL o datos de solicitudes, salida de consola, capturas de pantalla, adjuntos, datos de cuenta, dirección IP ni identidad. Se usa solo para diagnosticar fallos y errores del producto, nunca para análisis, publicidad, perfiles, IA o entrenamiento. Sentry conserva los eventos hasta 90 días en la región de la UE y puede usar sus subencargados publicados y el DPA y las Cláusulas Contractuales Tipo para transferencias ulteriores limitadas. Puedes retirar el consentimiento en cualquier momento para detener futuros informes. El servicio web de EVB Viewer no envía informes de errores del servidor.',
        },
        analytics: {
    ...
    'fr': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer est le responsable du traitement et utilise Functional Software, Inc. d/b/a Sentry comme sous-traitant pour les rapports d’erreur facultatifs du client. Les rapports de l’application de bureau et du navigateur ne sont envoyés qu’avec votre consentement. Un rapport peut contenir un Error ID aléatoire, la date, la gravité, un code de diagnostic, la version de l’application, le nom et la version du système d’exploitation et de l’environnement d’exécution, les cadres de pile d’EVB Viewer (fichier, fonction, ligne et colonne) ainsi que le type et le message de l’erreur une fois retirés les chemins locaux, noms de fichiers, URL et textes entre guillemets. Pour un plantage natif, il indique quel processus s’est arrêté et pourquoi ; le vidage mémoire n’est jamais envoyé. Il ne contient jamais de document ou de texte, de chemin ou journal local, d’URL ou donnée de requête, de sortie de console, de capture d’écran, de pièce jointe, de donnée de compte, d’adresse IP ou d’identité. Il sert uniquement à diagnostiquer les plantages et erreurs du produit, jamais à l’analyse, la publicité, au profilage, à l’IA ou à l’entraînement. Sentry conserve les événements jusqu’à 90 jours dans la région UE et peut recourir à ses sous-traitants publiés ainsi qu’au DPA et aux clauses contractuelles types pour des transferts ultérieurs limités. Vous pouvez retirer votre consentement à tout moment. Le service web EVB Viewer n’envoie aucun rapport d’erreur côté serveur.',
        },
        analytics: {
    ...
    'it': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer è il titolare del trattamento e usa Functional Software, Inc. d/b/a Sentry come responsabile per i report di errore opzionali del client. I report desktop e browser vengono inviati solo con il tuo consenso. Un report può contenere un Error ID casuale, data e ora, gravità, codice diagnostico, versione dell’app, nome e versione del sistema operativo e del runtime, frame dello stack di EVB Viewer (file, funzione, riga e colonna) e il tipo e il messaggio dell’errore dopo la rimozione di percorsi locali, nomi di file, URL e testo tra virgolette. In caso di arresto anomalo nativo registra quale processo è terminato e perché; il dump della memoria non viene mai inviato. Non contiene mai documenti o testo, percorsi o registri locali, URL o dati della richiesta, output della console, screenshot, allegati, dati dell’account, indirizzo IP o identità. Serve solo a diagnosticare arresti anomali ed errori del prodotto, mai per analisi, pubblicità, profilazione, IA o addestramento. Sentry conserva gli eventi per un massimo di 90 giorni nella regione UE e può usare i sub-responsabili pubblicati e il DPA e le Clausole Contrattuali Standard per trasferimenti successivi limitati. Puoi revocare il consenso in qualsiasi momento. Il servizio web di EVB Viewer non invia report di errore lato server.',
        },
        analytics: {
    ...
    'nl': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer is de verwerkingsverantwoordelijke en gebruikt Functional Software, Inc. d/b/a Sentry als verwerker voor optionele foutrapporten van de client. Desktop- en browserrapporten worden alleen met je toestemming verzonden. Een rapport kan een willekeurige Error ID, tijd, ernst, diagnostische code, appversie, naam en versie van besturingssysteem en runtime, stackframes van EVB Viewer (bestand, functie, regel en kolom) en het fouttype en de foutmelding bevatten nadat lokale paden, bestandsnamen, URL’s en geciteerde tekst zijn verwijderd. Bij een native crash legt het vast welk proces is gestopt en waarom; de geheugendump wordt nooit verzonden. Het bevat nooit documenten of tekst, lokale paden of logboeken, URL’s of aanvraaggegevens, console-uitvoer, schermafbeeldingen, bijlagen, accountgegevens, IP-adres of identiteit. Rapporten worden alleen gebruikt om crashes en productfouten op te lossen, nooit voor analyse, advertenties, profilering, AI of training. Sentry bewaart gebeurtenissen maximaal 90 dagen in de EU-regio en kan zijn gepubliceerde subverwerkers en de DPA en standaardcontractbepalingen gebruiken voor beperkte doorgiften. Je kunt je toestemming altijd intrekken. De webdienst van EVB Viewer verstuurt geen foutrapporten vanaf de server.',
        },
        analytics: {
    ...
    'pt': {
        ...
        diagnostics: {
            ...
            body: 'O EVB Viewer é o responsável pelo tratamento e utiliza a Functional Software, Inc. d/b/a Sentry como subcontratante para relatórios opcionais de erros do cliente. Os relatórios da aplicação desktop e do navegador só são enviados com o seu consentimento. Um relatório pode conter um Error ID aleatório, data e hora, gravidade, código de diagnóstico, versão da aplicação, nome e versão do sistema operativo e do ambiente de execução, quadros da pilha do EVB Viewer (ficheiro, função, linha e coluna) e o tipo e a mensagem do erro depois de removidos caminhos locais, nomes de ficheiros, URLs e texto entre aspas. Numa falha nativa, regista que processo terminou e porquê; o despejo de memória nunca é enviado. Nunca contém documentos ou texto, caminhos ou registos locais, URLs ou dados do pedido, saída da consola, capturas de ecrã, anexos, dados da conta, endereço IP ou identidade. Serve apenas para diagnosticar falhas e erros do produto, nunca para análise, publicidade, definição de perfis, IA ou treino. O Sentry conserva eventos por até 90 dias na região da UE e pode utilizar os subcontratantes publicados e o DPA e as Cláusulas Contratuais-Tipo para transferências ulteriores limitadas. Pode retirar o consentimento a qualquer momento. O serviço web do EVB Viewer não envia relatórios de erros do servidor.',
        },
        analytics: {
    ...
    'pt-BR': {
        ...
        diagnostics: {
            heading: 'Relatórios de erros opcionais do Sentry',
            body: 'O EVB Viewer é o controlador e usa a Functional Software, Inc. d/b/a Sentry como operadora para relatórios opcionais de erros do cliente. Os relatórios do aplicativo desktop e do navegador só são enviados com o seu consentimento. Um relatório pode conter um Error ID aleatório, data e hora, gravidade, código de diagnóstico, versão do aplicativo, nome e versão do sistema operacional e do ambiente de execução, quadros da pilha do EVB Viewer (arquivo, função, linha e coluna) e o tipo e a mensagem do erro depois que caminhos locais, nomes de arquivos, URLs e texto entre aspas são removidos. Em uma falha nativa, registra qual processo terminou e por quê; o despejo de memória nunca é enviado. Nunca contém documentos ou texto, caminhos ou registros locais, URLs ou dados da solicitação, saída do console, capturas de tela, anexos, dados da conta, endereço IP nem identidade. É usado somente para diagnosticar falhas e erros do produto, nunca para análise, publicidade, criação de perfil, IA ou treinamento. O Sentry mantém os eventos por até 90 dias na região da UE e pode usar seus suboperadores publicados e o DPA e as Cláusulas Contratuais Padrão para transferências posteriores limitadas. Você pode retirar o consentimento a qualquer momento. O serviço web do EVB Viewer não envia relatórios de erros do servidor.',
        },
        analytics: {
            heading: 'Dados analíticos e informações técnicas',
    ...
    'ru': {
        ...
        diagnostics: {
            ...
            body: 'EVB Viewer является контролёром и использует Functional Software, Inc. d/b/a Sentry как обработчика необязательных клиентских отчётов об ошибках. Десктопное приложение и браузер отправляют отчёты только с вашего согласия. Отчёт может содержать случайный Error ID, время, уровень серьёзности, код диагностики, версию приложения, название и версию операционной системы и среды выполнения, кадры стека EVB Viewer (файл, функция, строка и столбец), а также тип и сообщение ошибки после удаления локальных путей, имён файлов, URL и текста в кавычках. При нативном сбое фиксируется, какой процесс завершился и почему; дамп памяти никогда не отправляется. В нём никогда нет документов или текста, локальных путей или журналов, URL или данных запроса, вывода консоли, снимков экрана, вложений, данных учётной записи, IP-адреса или личности. Отчёты используются только для диагностики сбоев и ошибок продукта, а не для аналитики, рекламы, профилирования, ИИ или обучения. Sentry хранит события до 90 дней в регионе ЕС и может использовать опубликованных субобработчиков, DPA и Стандартные договорные положения для ограниченной дальнейшей передачи. Вы можете в любой момент отозвать согласие и остановить будущие отчёты. Веб-сервис EVB Viewer не отправляет серверные отчёты об ошибках.',
        },
        analytics: {
