...
- Concurrency: at most one agent changes viewer core (`app/modules/pdf-viewer`, `document-viewer`, `workspace-shell`, annotations) at a time. Elsewhere, agents may work in parallel on disjoint files, but at most six agents that push to `main` are active at once. A worker counts only if it pushes itself, and read-only agents never count. One task or program has at most three pushing agents; further workers hand their changes to one integrating agent. If a rebase brings in changes to files you touched, rerun the affected checks before pushing. Only the owner can change these limits, by giving a number for the task. Check active threads and branches on every fleet host before starting.
