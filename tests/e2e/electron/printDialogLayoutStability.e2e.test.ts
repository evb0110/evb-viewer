...
async function openPrintDialog(page: Page) {
    ...
    await page.waitForSelector('[role="dialog"] [role="radio"][value="range"]', {
        timeout: PRINT_DIALOG_TIMEOUT_MS,
        visible: true,
    });
    // The modal scales in; measure the settled box.
...
describe('Electron E2E - print dialog layout stability', () => {
    const sessionFixture = createElectronE2ESessionFixture({
        sessionName: () => `e2e-print-dialog-layout-${Date.now()}`,
        timeoutMs: PRINT_DIALOG_TIMEOUT_MS,
    });
    ...
    it('keeps its controls in place when the page range is chosen and rejected', async () => {
        ...
        const {page} = session;
        await page.setViewport(PRINT_DIALOG_VIEWPORT);

        const fixturePath = resolve(process.cwd(), 'tests', 'fixtures', 'electron', 'generated-text.pdf');
        await openPdfInApp(page, fixturePath, PRINT_DIALOG_TIMEOUT_MS);
        await waitForPdfLoaded(page, PRINT_DIALOG_TIMEOUT_MS);
        await waitForViewerInteractive(page, PRINT_DIALOG_TIMEOUT_MS);
        ...
        await page.waitForSelector('[role="dialog"] [role="radio"][value="range"][aria-checked="true"]', {timeout: PRINT_DIALOG_TIMEOUT_MS});
        ...
        await page.waitForSelector('[role="dialog"] input[aria-label="Page range"][aria-invalid="true"]', {timeout: PRINT_DIALOG_TIMEOUT_MS});
