...
const registries = new Set<{dispose: () => Promise<void>}>();
const teardownAssertions = new Set<() => void>();
...
function registry(retention = {eventReplayTtlMs: 30_000, terminalRecordTtlMs: 60_000}, now?: () => number, unbindOnSettlement = false) {
    const jobs = createMainJobRegistry<IProgress, IResult, IError>({
...
describe('createMainJobRegistry violations', {timeout: 20_000}, () => {
    ...
    afterEach(async () => {
        ...
        registries.clear();
        try {
            for (const assertion of teardownAssertions) assertion();
        } finally {
    ...
    it('does not run a job when a start signal is already aborted', async () => {
        ...
        expect(run).not.toHaveBeenCalled(); await handle.settled; expect(snapshotMainOperations()).toEqual([]);
    });
    ...
    it('closes subscriptions through owner loss and record disposal', async () => {
        ...
        teardownAssertions.add(() => expect(closedOnDisposal).toHaveBeenCalledOnce());
    });
    ...
    it('honors shutdown admission, abortable cancellation, and critical commit drain', async () => {
        ...
        critical.resolve({value: 'written'}); await expect(drain).resolves.toEqual({completed: true, pending: []});
    });
    it('cleans scratch on success, failure, cancellation, and owner loss', async () => {
        ...
        const unmanaged = mkdtempSync(join(mocks.appTempDir, 'unmanaged-')); expect(existsSync(unmanaged)).toBe(true);
    });
});
