...
describe('OCR catalog v3 refusal', () => {
    it('reads a v3 catalog as absent', async () => {
        const root = await mkdtemp(join(tmpdir(), 'evb-ocr-catalog-v3-'));
        roots.push(root);
        await writeFile(join(root, 'page-0001.json'), JSON.stringify(page));
        await writeFile(join(root, 'manifest.json'), JSON.stringify({
            version: 3,
            documentRevision: {token: revision},
            createdAt: Date.now(),
        ...
        await expect(openCatalog(root)).resolves.toBeNull();
    });
});
