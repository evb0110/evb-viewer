...
import type {
    IScanCleanupDetectionResult, IScanCleanupOptions, IScanCleanupPlacementAnchorCalibrationRequest, IScanCleanupDetectionRequest, IScanCleanupPreviewRequest, TScanCleanupDetectionJobState,
...
import {
    defaultDependencies, materializeScanCleanupPreviewRequest, scanCleanupPreviewLifecycle, type IScanCleanupPreviewService,
...
import {
    scanCleanupDetectionOwner, type IScanCleanupDetectionOwner,
...
import {
    copyFile, mkdir, mkdtemp, open, readdir, readFile, rm, stat, truncate, writeFile,
} from 'fs/promises';
...
export async function scenarioRasterizesDetectionPagesAsWideAsThe11CoreHostAllowsAndLeasesThatWidth(): Promise<void> {
    ...
    const originalRenderPage = deps.renderPage;
    const rasterBatchReady = Promise.withResolvers<undefined>();
